...
public class PersistentCacheManager implements PluggableSearchEngine {
  ...
  private static Timer timer = Metrics.newTimer(new MetricName(
      PersistentKafkaStreamDataProvider.class, "cachePersistTime"), TimeUnit.MILLISECONDS,
    TimeUnit.SECONDS);
  private static Counter numberOfBatchesCounter = Metrics.newCounter(new MetricName(
      PersistentKafkaStreamDataProvider.class, "numberOfBatches"));
  ...
  public void start(SenseiCore senseiCore) {
    ...
    for (int partition : senseiCore.getPartitions()) {
      ...
      PersistentCache persistentCache = new PersistentCache(getPath(indexDirectory, nodeId,
        partition), versionComparator);
