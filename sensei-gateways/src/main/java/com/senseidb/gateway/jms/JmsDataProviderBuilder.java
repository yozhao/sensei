...
public class JmsDataProviderBuilder extends SenseiGateway<Message> {
  ...
  public StreamDataProvider<JSONObject> buildDataProvider(
      final DataSourceFilter<Message> dataFilter, String oldSinceKey,
    ...
    if (topicFactory == null) topicFactory = pluginRegistry.getBeanByFullPrefix(topicFac,
      TopicFactory.class);
    ...
    TopicConnectionFactory connectionFactory = pluginRegistry.getBeanByName(
      config.get("jms.connectionFactory"), TopicConnectionFactory.class);

    if (connectionFactory == null) connectionFactory = pluginRegistry.getBeanByFullPrefix(
      config.get("jms.connectionFactory"), TopicConnectionFactory.class);
    ...
    DataEventBuilder<JSONObject> dataEventBuilder = new DataEventBuilder<JSONObject>() {
      ...
      @Override
      public DataEvent<JSONObject> buildDataEvent(Message message) throws JMSException {

        try {
          return new DataEvent<JSONObject>(filter.filter(message),
    ...
    JMSStreamDataProvider<JSONObject> provider = new JMSStreamDataProvider<JSONObject>(topic,
        clientID, connectionFactory, topicFactory, dataEventBuilder, _versionComparator);
    return provider;
  }
