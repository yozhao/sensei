...
class RabbitMQConsumerManager {
    ...
    RabbitMQConsumerManager(RabbitMQConfig config, RabbitMQStreamDataProvider rabbitMQStreamDataProvider) {
    ...
    synchronized void start() throws IOException {
        ...
        for (int i = 0; i < _rabbitMQConfig.getConsumeWorkerCount(); i++) {
            _consumers[i] = new RabbitMQConsumer(_rabbitMQConfig.getConnectionConfig().getHost() + ":" + i, this, _connection,
                    _rabbitMQConfig, _rabbitMQStreamDataProvider);
