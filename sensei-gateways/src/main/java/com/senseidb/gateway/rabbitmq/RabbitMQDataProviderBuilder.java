...
public class RabbitMQDataProviderBuilder extends SenseiGateway<byte[]> {
  ...
  private void initRabbitMQConfigs() {
    ...
    for (int i = 0; i < _mqConfigs.length; i++) {
      ...
      if (StringUtils.isEmpty(rabbitmqServer)) continue;
      ...
      int portInt = -1;
      if (StringUtils.isNotEmpty(port)) try {
      ...
      if (StringUtils.isNotEmpty(exchangeName)) {
        ...
        exchangeConfig = new ExchangeConfig(exchangeName, exchangeType,
            Boolean.valueOf(exchangeDurable));
      }
      ...
      boolean queueExclusive = Boolean.valueOf(config.get(rabbitmqServer
          + RABBIT_MQ_QUEUE_EXCLUSIVE));
      boolean queueAutoDelete = Boolean.valueOf(config.get(rabbitmqServer
          + RABBIT_MQ_QUEUE_AUTO_DELETE));
      String queueRoutingKey = config.get(rabbitmqServer + RABBIT_MQ_QUEUE_ROUTING_KEY);
      if (StringUtils.isNotEmpty(queueRoutingKey)) queueConfig = new QueueConfig(queueName,
          queueDurable, queueExclusive, queueAutoDelete, queueRoutingKey);
      else queueConfig = new QueueConfig(queueName, queueDurable, queueExclusive, queueAutoDelete);

      _mqConfigs[i] = new RabbitMQConfig(connectionConfig, exchangeConfig, queueConfig);

      try {
        _mqConfigs[i].setConsumeWorkerCount(Integer.parseInt(config.get(rabbitmqServer
            + RABBIT_MQ_CONSUMER_COUNT)));
      } catch (NumberFormatException e) {
        // do nothing
      }
  ...
  public StreamDataProvider<JSONObject> buildDataProvider(DataSourceFilter<byte[]> dataFilter,
      ...
      throws Exception {
    initRabbitMQConfigs();
    return new RabbitMQStreamDataProvider(DEFAULT_VERSION_COMPARATOR, _mqConfigs, dataFilter,
        Oldsincekey);
  }
