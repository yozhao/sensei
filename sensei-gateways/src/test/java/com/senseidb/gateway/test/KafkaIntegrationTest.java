...
public class KafkaIntegrationTest {
  private static File kafkaServerFile = new File(
      "src/test/resources/configs/kafka-server.properties");
  ...
  public static void init() throws Exception {
    ...
    indexDir.mkdir();
    ConfDir1 = new File(KafkaIntegrationTest.class.getClassLoader()
        .getResource("kafka-sensei-conf").toURI());
    ...
    SenseiPluginRegistry senseiPluginRegistry = SenseiPluginRegistry
        .build(new PropertiesConfiguration(new File(ConfDir1, "sensei.properties")));
    ...
    for (String json : FileUtils.readLines(new File(
        "src/test/resources/kafka-sensei-conf/data/cars.json"))) {
  ...
  public void test1DataFromPersistentCache() {
    PersistentCache persistentCache = new PersistentCache(PersistentCacheManager.getPath(
      indexDirectory, 1, 0), ZoieConfig.DEFAULT_VERSION_COMPARATOR);
    List<Pair<String, String>> eventsNotAvailableInZoie = persistentCache
        .getEventsNotAvailableInZoie("0");
