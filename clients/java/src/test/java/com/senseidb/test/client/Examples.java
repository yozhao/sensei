...
public class Examples {
  public static void main(String[] args) throws Exception {
    ...
    // ORDER BY price desc LIMIT 0,10 BROWSE BY color(true, 1, 10, hits), year(true, 1, 10, value),
    // price
    ...
    SenseiClientRequest senseiRequest = SenseiClientRequest
        ...
        .addFacet(
          "color",
          Facet.builder().minHit(1).expand(true).orderByHits().max(10)
              .addProperty("maxFacetsPerKey", "3").build())
        ...
        .addSelection(
          Selection.terms("tags", Arrays.asList("cool", "hybrid"), Arrays.asList("favorite"),
            Operator.and))
        .addSelection(
    ...
    SenseiResult senseiResult = new SenseiServiceProxy("localhost", 8080)
        .sendSearchRequest(senseiRequest);
  ...
  public static SenseiClientRequest.Builder basicWithSelections(SenseiClientRequest.Builder builder) {
    builder.paging(5, 2).groupBy(7, "car", "year")
        ...
        .addTermVector("Term1").explain(true).partitions(Arrays.asList(1, 2));
    return builder;
  }
  ...
  public static SenseiClientRequest.Builder filters(SenseiClientRequest.Builder builder) {
    ...
    Filters.and(
      ...
      Filters.and(Filters.or(Filters.term("field", "value"),
      ...
      Filters.range("field1", "*", "*")), Filters.or(
      Filters.ids(Arrays.asList("val2", "val3"), Arrays.asList("ExcludedValue")),
      Filters.range("field1", "*", "*", true, true)), Filters.ids(Arrays.asList("val2", "val3"),
      Arrays.asList("ExcludedValue")), Filters.term("field", "value"), Filters.boolShould(Filters
        .or(Filters.term("field", "value")))));
    return builder;
  }

  public static SenseiClientRequest.Builder queries(SenseiClientRequest.Builder builder) {
    List<Query> innerQueries = Arrays.asList(Queries.matchAllQuery(3), Queries.disMax(2.0, 1.0,
      Queries.term("field1", "value1", 1.0)), Queries.ids(Arrays.asList("val2", "val3"),
      Arrays.asList("ExcludedValue"), 1.0), Queries.matchAllQuery(3.0), Queries.path("field",
      "/path1/path2", 1.0), Queries.prefix("field", "prefix", 2.0), Queries.wildcard("field",
      "wildcard", 3.0), Queries.spanFirst(Queries.spanTerm("field", "val", 3.5), 3, 1.0), Queries
        .spanNear(Arrays.asList(Queries.spanTerm("field", "val", 3.5)), 3, true, true, 1.0),
      Queries.spanNot(Queries.spanTerm("field", "val", 3.5),
        Queries.spanTerm("field", "val2", 3.5), 1.0), Queries.spanOr(1.0,
        Queries.spanTerm("field", "val", 3.5)), Queries.spanTerm("field", "val", 3.5), Queries
          .textQuery("column", "text", Operator.or, Type.phrase, 1.0),
      Queries.stringQueryBuilder().query("").autoGeneratePhraseQueries(true).defaultField("field")
          .defaultOperator(Operator.and).fields("field1", "field2").tieBreaker(2).build());
