...
public class JsonDeserializer {
    ...
    try {
      ...
      for (Field field : cls.getDeclaredFields()) {
        ...
        if (type == Integer.class) {
          ...
          value = deserializeArray((isParameterizedType ? getGenericType(genericType, 0) : null),
            jsonArray);
  ...
  private static Map deserializeMap(Type genericType, JSONObject mapJson) throws Exception {
    ...
    for (String paramName : names) {
      ...
      if (JsonSerializer.NULL.equals(paramName)) paramName = null;
      if (mapValue == null) {
        ...
        if (valueType instanceof ParameterizedType) {
          ...
          && Map.class.isAssignableFrom(((Class) ((ParameterizedType) valueType).getRawType()))) {
