...
public class SelectionJsonHandler implements JsonHandler<Selection> {
  ...
  static {
    for (Class<? extends Selection> cls : Arrays.asList(Term.class, Terms.class, Path.class,
      Range.class, Selection.Custom.class)) {
  ...
  public Selection deserialize(JSONObject json) throws JSONException {
    ...
    if (names.length == 0 || !selectionClasses.keySet().contains(names[0])) {
      throw new IllegalStateException("The json object doesn't contain the value from "
    ...
    Selection selection = JsonDeserializer.deserialize(selectionClasses.get(name),
      innerPart.getJSONObject(fieldName), false);
