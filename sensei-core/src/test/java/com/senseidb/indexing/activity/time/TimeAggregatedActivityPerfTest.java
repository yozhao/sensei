...
public class TimeAggregatedActivityPerfTest extends Assert {
  ...
  public void test1Perf10mInsertsAndUpdateAfterwards() throws Exception {
    ...
    activityValues.syncWithPersistentVersion(String.valueOf((numOfEvents - 1) * recordsCount
        + recordsCount - 1));
    System.out.println("persistentTime = " + (System.currentTimeMillis() - insertTime));
    assertEquals(numOfEvents,
      timeAggregatedActivityValues.getValuesMap().get("5m").fieldValues[50000]);
    assertEquals(numOfEvents,
      timeAggregatedActivityValues.getValuesMap().get("10m").fieldValues[50000]);
    assertEquals(numOfEvents,
      timeAggregatedActivityValues.getValuesMap().get("10m").fieldValues[90000]);
