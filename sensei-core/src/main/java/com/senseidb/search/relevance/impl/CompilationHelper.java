...
public class CompilationHelper {
  ...
  private static String SCORE_METHOD_HEADER = "public float score(short[] shorts, "
      + "int[] ints, " + "long[] longs, " + "float[] floats, " + "double[] doubles, "
      + "boolean[] booleans, " + "String[] strings, " + "Set[] sets, " + "Map[] maps, "
  ...
  static {
    ...
    // the first int in the following int[] is the index of the string template above
  ...
  public static CustomMathModel createCustomMathScorer(JSONObject jsonModel, DataTable dataTable)
    ...
    if (jsonModel == null) throw new RelevanceException(ErrorType.JsonParsingError,
        "No json model is specified.");
    ...
    int[] facetIndice = new int[] { 0, 0, 0 }; // store the facetIndex, facetMultiIndex, and
                                               // activity engine facet Index;
    ...
    while (it_var.hasNext()) {
      ...
      for (int i = 0; i < varArray.length(); ++i) {
        ...
        if (symbol.equals(RelevanceJSONConstants.KW_INNER_SCORE)
          ...
          throw new RelevanceException(ErrorType.JsonParsingError, "Internal variable name, "
              + symbol + ", is reserved.");
        }

        Integer typeNum = RelevanceJSONConstants.VARIABLE_INFO_MAP.get(type);
        if (typeNum == null) {
          throw new RelevanceException(ErrorType.JsonParsingError, "Variable type, " + type
              + ", is not recognized.");
        }
    ...
    if (dataTable.funcBody == null || dataTable.funcBody.length() == 0) throw new RelevanceException(
        ErrorType.JsonParsingError, "No function body found.");

    if (dataTable.funcBody.indexOf("return ") == -1) throw new RelevanceException(
        ErrorType.JsonParsingError, "No return statement in the function body.");
    ...
    for (int i = 0; i < dataTable.lls_params.size(); i++) {
      ...
      if (!dataTable.hm_type.containsKey(symbol)) throw new RelevanceException(
          ErrorType.JsonParsingError, "function parameter: " + symbol + " was not defined.");
      ...
      if (typeNum >= RelevanceJSONConstants.TYPENUMBER_FACET_INT
        ...
        if ((!dataTable.hm_symbol_facet.containsKey(symbol))
            && (!dataTable.hm_symbol_mfacet.containsKey(symbol))) throw new RelevanceException(
            ErrorType.JsonParsingError, "function parameter: " + symbol + " was not defined.");
      }
    }
      ...
      synchronized (CompilationHelper.class) {
        ...
        try {
          ...
          if (hmModels.containsKey(className)) {
            ...
            logger.info(e.getMessage());
            throw new RelevanceException(ErrorType.JsonCompilationError,
                "Compilation error of json relevance model.", e);
          }
        }
        ...
        try {
          ...
          logger.info(e.getMessage());
          throw new RelevanceException(ErrorType.JsonCompilationError,
              "Instantiation exception of relevance object.", e);
        } catch (IllegalAccessException e) {
          logger.info(e.getMessage());
          throw new RelevanceException(ErrorType.JsonCompilationError,
              "Instantiation exception of relevance object; Illegal access exception", e);
        }

        if (hmModels.size() > MAX_NUM_MODELS) hmModels = new HashMap<String, CustomMathModel>();
  ...
  public static void initializeValues(JSONObject jsonValues, DataTable dataTable)
    ...
    while (it.hasNext()) {
      ...
      if (typeNum == RelevanceJSONConstants.TYPENUMBER_INNER_SCORE
          || typeNum == RelevanceJSONConstants.TYPENUMBER_NOW) continue;
      ...
      if (typeNum >= RelevanceJSONConstants.TYPENUMBER_SET_INT
        ...
        if (values == null) throw new JSONException("Variable " + symbol + " does not have value.");
        ...
        // denote if the map is represented in a way of combination of key jsonarray and value
        // jsonarray;
        ...
        if (isKeyValue) {
          ...
          if (keysArrayList == null) throw new JSONException("Variable " + symbol
              + " is a map, but does not have a key list.");

          if (valuesArrayList == null) throw new JSONException("Variable " + symbol
              + "is a map, but does not have a value list.");

          keyArraySize = keysArrayList.length();
          valueArraySize = valuesArrayList.length();
          if (keyArraySize != valueArraySize) throw new JSONException("Variable " + symbol
              + ": key size is different from value size, can not convert to a map.");
        ...
        case RelevanceJSONConstants.TYPENUMBER_MAP_INT_INT:
          ...
          for (int j = 0; j < keySize; j++) {
            if (isKeyValue) ((Int2IntOpenHashMap) hm).put(keysArrayList.getInt(j),
              valuesArrayList.getInt(j));
            else ((Int2IntOpenHashMap) hm).put(keysList.getInt(j),
              values.getInt(keysList.getString(j)));
          }
          break;
        case RelevanceJSONConstants.TYPENUMBER_MAP_INT_DOUBLE:
          hm = new Int2DoubleOpenHashMap();
          for (int j = 0; j < keySize; j++) {
            if (isKeyValue) ((Int2DoubleOpenHashMap) hm).put(keysArrayList.getInt(j),
              valuesArrayList.getDouble(j));
            else ((Int2DoubleOpenHashMap) hm).put(keysList.getInt(j),
              values.getDouble(keysList.getString(j)));
          }
          break;
        case RelevanceJSONConstants.TYPENUMBER_MAP_INT_FLOAT:
          hm = new Int2FloatOpenHashMap();
          for (int j = 0; j < keySize; j++) {
            if (isKeyValue) ((Int2FloatOpenHashMap) hm).put(keysArrayList.getInt(j),
              (float) valuesArrayList.getDouble(j));
            else ((Int2FloatOpenHashMap) hm).put(keysList.getInt(j),
              (float) values.getDouble(keysList.getString(j)));
          }
          break;
        case RelevanceJSONConstants.TYPENUMBER_MAP_INT_LONG:
          hm = new Int2LongOpenHashMap();
          for (int j = 0; j < keySize; j++) {
            if (isKeyValue) ((Int2LongOpenHashMap) hm).put(keysArrayList.getInt(j),
              Long.parseLong(valuesArrayList.getString(j)));
            else ((Int2LongOpenHashMap) hm).put(keysList.getInt(j),
              Long.parseLong(values.getString(keysList.getString(j))));
          }
          break;
        case RelevanceJSONConstants.TYPENUMBER_MAP_INT_STRING:
          hm = new Int2ObjectOpenHashMap();
          for (int j = 0; j < keySize; j++) {
            if (isKeyValue) ((Int2ObjectOpenHashMap) hm).put(keysArrayList.getInt(j),
              valuesArrayList.getString(j));
            else ((Int2ObjectOpenHashMap) hm).put(keysList.getInt(j),
              values.getString(keysList.getString(j)));
          }
          break;
        case RelevanceJSONConstants.TYPENUMBER_MAP_STRING_INT:
          hm = new Object2IntOpenHashMap();
          for (int j = 0; j < keySize; j++) {
            if (isKeyValue) ((Object2IntOpenHashMap) hm).put(keysArrayList.getString(j),
              valuesArrayList.getInt(j));
            else {
        ...
        case RelevanceJSONConstants.TYPENUMBER_MAP_STRING_DOUBLE:
          ...
          for (int j = 0; j < keySize; j++) {
            if (isKeyValue) ((Object2DoubleOpenHashMap) hm).put(keysArrayList.getString(j),
              valuesArrayList.getDouble(j));
            else {
              String key = keysList.getString(j);
        ...
        case RelevanceJSONConstants.TYPENUMBER_MAP_STRING_FLOAT:
          ...
          for (int j = 0; j < keySize; j++) {
            if (isKeyValue) ((Object2FloatOpenHashMap) hm).put(keysArrayList.getString(j),
              (float) valuesArrayList.getDouble(j));
            else {
              String key = keysList.getString(j);
        ...
        case RelevanceJSONConstants.TYPENUMBER_MAP_STRING_LONG:
          ...
          for (int j = 0; j < keySize; j++) {
            if (isKeyValue) ((Object2LongOpenHashMap) hm).put(keysArrayList.getString(j),
              Long.parseLong(valuesArrayList.getString(j)));
            else {
              String key = keysList.getString(j);
              ((Object2LongOpenHashMap) hm).put(key,
                Long.parseLong(values.getString(keysList.getString(j))));
            }
          }
          break;
        case RelevanceJSONConstants.TYPENUMBER_MAP_STRING_STRING:
          ...
          for (int j = 0; j < keySize; j++) {
            if (isKeyValue) ((Object2ObjectOpenHashMap) hm).put(keysArrayList.getString(j),
              valuesArrayList.getString(j));
            else {
              String key = keysList.getString(j);
        ...
        if (!jsonValues.has(symbol)) throw new JSONException("Symbol " + symbol
            + " was not assigned with a value.");
        ...
        if (obj != null) dataTable.hm_var.put(symbol, obj);
        else throw new JSONException("function parameter: " + symbol
            + " can not be initialized as a custom Object.");
      }
    }
  ...
  private static LinkedList<String> filterParameters(DataTable dataTable) {
    ...
    for (String param : dataTable.lls_params) {
      if (!(dataTable.funcBody.indexOf(param) == -1)) lls_new.add(param);
    }
    return lls_new;
  }
  ...
  private static void handleFacetSymbols(String facetType, JSONArray facetArray, int[] facetIndice,
      DataTable dataTable) throws JSONException {
    Integer[] facetInfo = RelevanceJSONConstants.FACET_INFO_MAP.get(facetType);
    if (facetInfo == null) {
      String errorString = String
          .format(
            "Wrong facet type in facet variable definition json: %s. Map contents are %s. Facet array is %s.",
    ...
    for (int i = 0; i < facetArray.length(); i++) {
      ...
      if (dataTable.hm_symbol_facet.containsKey(symbol)
          || dataTable.hm_symbol_mfacet.containsKey(symbol)
          || dataTable.hm_symbol_afacet.containsKey(symbol)) throw new JSONException(
          "facet Symbol " + symbol + " already defined.");

      if (dataTable.hm_facet_index.containsKey(facetName)
          || dataTable.hm_mfacet_index.containsKey(facetName)
          || dataTable.hm_afacet_index.containsKey(facetName)) throw new JSONException(
          "facet name " + facetName + " already assigned to a symbol.");
  ...
  private static String makeFuncString(DataTable dataTable) throws JSONException {
    ...
    dataTable.useInnerScore = false; // set using innerscore to false at the beginning; once we see
                                     // innerscore is used in the function below, it will be set to
                                     // true;

    for (int i = 0; i < dataTable.lls_params.size(); i++) {
      String paramName = dataTable.lls_params.get(i);

      if (!dataTable.hm_type.containsKey(paramName) || (dataTable.hm_type.get(paramName) == null)) throw new JSONException(
          "function parameter " + paramName + " is not defined.");
      ...
      if (paramType.intValue() == RelevanceJSONConstants.TYPENUMBER_CUSTOM_OBJ) {
        ...
        if (className == null) throw new JSONException("Custom external object " + paramName
            + " is not found.");
        ...
        sb.append(String.format(PARAM_FORMAT_STRINGS[paramInfo[0]], className2, paramName,
          className2, paramIndices[paramInfo[1]]++));
      } else sb.append(String.format(PARAM_FORMAT_STRINGS[paramInfo[0]], paramName,
        paramIndices[paramInfo[1]]++));
  ...
  public static class CustomLoader extends ClassLoader {
    ...
    public Class<?> loadClass(String name) throws ClassNotFoundException {
      ...
      if (hs_safe.contains(name) || name.equals(_target)) return _cl.loadClass(name);
      else {
        String message = String.format("Unable to load class %s. Safe classes are %s", name,
          mkString(hs_safe));
  ...
  public static class DataTable {
    ...
    public boolean useInnerScore = true; // by default will calculate innerscore value, set this to
                                         // false will ignore inner score to save time;
