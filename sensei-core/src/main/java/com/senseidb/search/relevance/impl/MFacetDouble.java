...
public class MFacetDouble extends MFacet {
  ...
  public boolean containsAny(Object set) {
    ...
    for (int i = 0; i < this._length; i++)
      if (setDouble.contains(((TermDoubleList) _mTermList).getPrimitiveValue(_buf[i]))) return true;
  ...
  public boolean contains(double target) {
    for (int i = 0; i < this._length; i++)
      if (((TermDoubleList) _mTermList).getPrimitiveValue(_buf[i]) == target) return true;
