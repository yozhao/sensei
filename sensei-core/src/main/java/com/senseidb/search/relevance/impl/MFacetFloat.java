...
public class MFacetFloat extends MFacet {
  ...
  public boolean containsAny(Object set) {
    ...
    for (int i = 0; i < this._length; i++)
      if (setFloat.contains(((TermFloatList) _mTermList).getPrimitiveValue(_buf[i]))) return true;
  ...
  public boolean contains(float target) {
    for (int i = 0; i < this._length; i++)
      if (((TermFloatList) _mTermList).getPrimitiveValue(_buf[i]) == target) return true;
