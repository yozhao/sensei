...
public class RuntimeRelevanceFunction extends CustomRelevanceFunction {
  ...
  public RuntimeRelevanceFunction(CustomMathModel cModel, DataTable dt) {
    _cModel = cModel;
    _dt = dt;
  }
  ...
  private void initialRunningData(BoboSegmentReader boboReader, CustomMathModel cModel,
    ...
    while (iter_facet.hasNext()) {
      ...
      if (!(dataObj instanceof FacetDataCache<?>)) throw new IllegalArgumentException("Facet "
    ...
    while (iter_afacet.hasNext()) {
      ...
      if (!(arHandler instanceof ActivityRangeFacetHandler)) throw new IllegalArgumentException(
          "Facet " + afacetName + " is not an ActivityRangeFacetHandler.");
    ...
    final int[] facetIndex = new int[paramSize]; // if this parameter is a facet, what is its index
                                                 // number in the facet data array;
    final int[] arrayIndex = new int[paramSize]; // for each parameter, what is its index number in
                                                 // its own parameter array when passing into the
                                                 // function;
    final int[] mFacetIndex = new int[paramSize]; // if this parameter is a multi-facet, we need to
                                                  // know its index. Since we only use one array to
                                                  // store multi-facet, we do not need array index
                                                  // like the one for the simple facet;
    final int[] mArrayIndex = new int[paramSize]; // for each multi-facet, what is its index number
                                                  // in its own parameter array when passing into
                                                  // the function;
  ...
  private void updateArrayIndex(DataTable _dt, int paramSize, int[] types, int[] facetIndex,
      int[] arrayIndex, int[] mFacetIndex, int[] mArrayIndex, int[] aFacetIndex) {
  ...
  public float newScore(float innerScore, int docID) {
    ...
    for (int j = 0; j < dynamicAR.length; j++) {
      ...
      // only when the parameter is inner score variable or facet variable, we need to update the
      // score function input parameter arrays;
      ...
      // normal facet;
      case RelevanceJSONConstants.TYPENUMBER_FACET_INT:
        ints[_arrayIndex[dynamicAR[j]]] = ((TermIntList) _termLists[_facetIndex[dynamicAR[j]]])
            .getPrimitiveValue(_orderArrays[_facetIndex[dynamicAR[j]]].get(docID));
        break;
      case RelevanceJSONConstants.TYPENUMBER_FACET_LONG:
        longs[_arrayIndex[dynamicAR[j]]] = ((TermLongList) _termLists[_facetIndex[dynamicAR[j]]])
            .getPrimitiveValue(_orderArrays[_facetIndex[dynamicAR[j]]].get(docID));
        break;
      case RelevanceJSONConstants.TYPENUMBER_FACET_DOUBLE:
        doubles[_arrayIndex[dynamicAR[j]]] = ((TermDoubleList) _termLists[_facetIndex[dynamicAR[j]]])
            .getPrimitiveValue(_orderArrays[_facetIndex[dynamicAR[j]]].get(docID));
        break;
      case RelevanceJSONConstants.TYPENUMBER_FACET_FLOAT:
        floats[_arrayIndex[dynamicAR[j]]] = ((TermFloatList) _termLists[_facetIndex[dynamicAR[j]]])
            .getPrimitiveValue(_orderArrays[_facetIndex[dynamicAR[j]]].get(docID));
        break;
      case RelevanceJSONConstants.TYPENUMBER_FACET_SHORT:
        shorts[_arrayIndex[dynamicAR[j]]] = ((TermShortList) _termLists[_facetIndex[dynamicAR[j]]])
            .getPrimitiveValue(_orderArrays[_facetIndex[dynamicAR[j]]].get(docID));
        break;
      case RelevanceJSONConstants.TYPENUMBER_FACET_STRING:
        strings[_arrayIndex[dynamicAR[j]]] = ((TermStringList) _termLists[_facetIndex[dynamicAR[j]]])
            .get(_orderArrays[_facetIndex[dynamicAR[j]]].get(docID));
        break;
    ...
    return _cModel.score(shorts, ints, longs, floats, doubles, booleans, strings, sets, maps,
      mFacetInts, mFacetLongs, mFacetFloats, mFacetDoubles, mFacetShorts, mFacetStrings, objs);
  }
