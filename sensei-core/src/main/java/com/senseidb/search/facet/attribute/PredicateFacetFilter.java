...
public class PredicateFacetFilter extends RandomAccessFilter {
  ...
  public RandomAccessDocIdSet getRandomAccessDocIdSet(BoboSegmentReader reader) throws IOException {
    ...
    int endDocIdTemp = -1;
    for (int i = facetPredicate.valueStartIndex(facetDataCache); i < facetPredicate
        .valueEndIndex(facetDataCache); i++) {
  ...
  public double getFacetSelectivity(BoboSegmentReader reader) {
    ...
    for (int i = facetPredicate.valueStartIndex(dataCache); i < facetPredicate
        .valueEndIndex(dataCache); i++) {
