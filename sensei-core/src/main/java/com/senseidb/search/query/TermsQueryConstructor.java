...
public class TermsQueryConstructor extends QueryConstructor {
  ...
  protected Query doConstructQuery(JSONObject json) throws JSONException {
    ...
    if (!iter.hasNext()) throw new IllegalArgumentException("no term value specified: " + json);
    ...
    if (values != null && values.length() > 0) {
      for (int i = 0; i < values.length(); ++i) {
        if (AND_PARAM.equals(op)) {
          ...
          query
              .add(new TermQuery(new Term(field, values.getString(i))), BooleanClause.Occur.SHOULD);
        }
      }
    ...
    if (excludes != null) {
      for (int i = 0; i < excludes.length(); ++i) {
        query.add(new TermQuery(new Term(field, excludes.getString(i))),
