...
public abstract class QueryConstructor {
  ...
  static {
    ...
    QUERY_CONSTRUCTOR_MAP
        .put(SpanFirstQueryConstructor.QUERY_TYPE, new SpanFirstQueryConstructor());
  ...
  public static QueryConstructor getQueryConstructor(String type, QueryParser qparser) {
    ...
    if (queryConstructor == null) {
      if (QueryStringQueryConstructor.QUERY_TYPE.equals(type)) queryConstructor = new QueryStringQueryConstructor(
          qparser);
      else if (TextQueryConstructor.QUERY_TYPE.equals(type)) queryConstructor = new TextQueryConstructor(
          qparser);
  ...
  public static Query constructQuery(JSONObject jsonQuery, QueryParser qparser)
      throws JSONException, IOException {
    if (jsonQuery == null) return null;
    ...
    if (queryConstructor == null) throw new IllegalArgumentException("Query type '" + type
        + "' not supported");
    ...
    if (jsonValue.has(RELEVANCE)) {
      ...
      if (jsonRelevance == null) throw new JSONException(
          "relevance part of the query json can not be parsed.");
    }
    if (jsonRelevance == null) {
      ...
      if (func == null) throw new JSONException("Can not create the score function;");

      return new ScoreAugmentQuery(baseQuery, func, valuesJson);
    }
  }

  abstract protected Query doConstructQuery(JSONObject jsonQuery) throws JSONException, IOException;
}
