...
public class ConstExpQueryConstructor extends QueryConstructor {
  ...
  // "const_exp" : {
  // "lvalue" : 4,
  // "operator" : "in", // supported operations: (1) set operations: in, not_in, ==, != ; (2) single
  // value operations: >, >=, ==, <, <=, !=,
  // "rvalue" : [4,5,6]
  // },
  //
  ...
  // ">, >=, <, <=," are normal boolean operations, applied to simple numerical value; (such as an
  // integer or double)
  // "!=, ==" can be applied to both simple numerical value and set and string;
  //
  // for set operations in or not_in, left value could be a single element, right side has to be a
  // collection.
  //
  // Expression Query is mostly combined with other queries to form a boolean query, and filled by
  // query template.
  ...
  protected Query doConstructQuery(JSONObject json) throws JSONException {
    ...
    if (!iter.hasNext()) throw new IllegalArgumentException(
        "no operator or values specified in ExpressionQuery: " + json);

    while (iter.hasNext()) {
      String field = iter.next();
      if (field.equals(QueryConstructor.OPERATOR_PARAM)) operator = json.optString(field);
      else if (field.equals(QueryConstructor.LEFT_VALUE)) lvalue = json.opt(field);
      else if (field.equals(QueryConstructor.RIGHT_VALUE)) rvalue = json.opt(field);
    }

    if (operator == null) throw new IllegalArgumentException(
        "operator not defined in ExpressionQuery: " + json);
    if (lvalue == null) throw new IllegalArgumentException(
        "left value not defined in ExpressionQuery: " + json);
    if (rvalue == null) throw new IllegalArgumentException(
        "right value not defined in ExpressionQuery: " + json);
    ...
    if (lvalue instanceof JSONObject) lvalue = getValueFromFunction((JSONObject) lvalue);
    if (rvalue instanceof JSONObject) rvalue = getValueFromFunction((JSONObject) rvalue);
    ...
    // single value comparisons only;
      ...
      if (lvalue instanceof JSONArray || rvalue instanceof JSONArray) throw new IllegalArgumentException(
          "operator " + operator + " is not defined for list, in ExpressionQuery: " + json);
      ...
      throw new IllegalArgumentException("Operator " + operator
          + " is not supported in ExpressionQuery: " + json);
    }
  ...
  private double convertToDouble(Object value, JSONObject json) {
    if (value instanceof Number) {
      return ((Number) value).doubleValue();
        ...
        "operator >, >=, <, <= can only be applied to double, int, long or float type data, in ExpressionQuery: "
            + json);
  }
  ...
  private Object getValueFromFunction(JSONObject funcJSON) throws JSONException {
    ...
    if (function.length() == 0) throw new IllegalArgumentException(
        "No function name is defined in ExpressionQuery's function json: " + funcJSON);

    JSONArray params = funcJSON.optJSONArray(QueryConstructor.PARAMS_PARAM);
    if (params == null) throw new IllegalArgumentException(
        "No function param is defined in ExpressionQuery's function json: " + funcJSON);
    ...
    if (function.equals("length")) {
      ...
      if (param == null) throw new IllegalArgumentException("No param is provided for function '"
          + function + "' defined in ExpressionQuery's function json: " + funcJSON);
      return (double) param.length();
    } else throw new IllegalArgumentException("Unsupported function '" + function
        + "' in ExpressionQuery's function json: " + funcJSON);
  }
  ...
  private boolean checkIn(Object lvalue, Object rvalue, JSONObject json) throws JSONException {
    ...
    if (rvalue instanceof JSONArray) {
      ...
      } else if (hs.contains(lvalue)) bool = true;
      else bool = false;
    } else {
      throw new IllegalArgumentException(
          "operator not_in requires a list of objects as the right value, in ExpressionQuery: "
              + json);
    }
    return bool;
  }
  ...
  private boolean checkEqual(Object lvalue, Object rvalue, JSONObject json) throws JSONException {
    ...
    if ((lvalue instanceof JSONArray) && (rvalue instanceof JSONArray)) {
      ...
      if (lvalue instanceof String && rvalue instanceof String) bool = lvalue.equals(rvalue);
      else {
        double ldouble = convertToDouble(lvalue, json);
        double rdouble = convertToDouble(rvalue, json);
        bool = ldouble == rdouble;
        ...
        "for == operator, left value and right value should be both simple values or both lists. in ExpressionQuery: "
            + json);
