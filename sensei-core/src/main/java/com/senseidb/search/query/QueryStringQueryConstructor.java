  ...
  protected Query doConstructQuery(JSONObject jsonQuery) throws JSONException {
    ...
    try {
      for (String name : JSONObject.getNames(jsonQuery)) {
        if (QUERY_PARAM.equals(name)) continue;
        if (BOOST_PARAM.equals(name)) continue;

        List<String> fields = new ArrayList<String>();
        String defaultField = jsonQuery.optString(DEFAULT_FIELD_PARAM, null);
        if (defaultField != null && defaultField.length() != 0) fields.add(defaultField);

        JSONArray fieldsArray = jsonQuery.optJSONArray(FIELDS_PARAM);
        if (fieldsArray != null) {
          for (int i = 0; i < fieldsArray.length(); ++i) {
            String field = fieldsArray.optString(i, null);
            if (field != null && field.length() != 0) fields.add(field);
          }
        }

        if (fields.size() == 0) fields.add("contents");
        ...
        boolean lowercase_expanded_terms = jsonQuery.optBoolean(LOWERCASE_EXPANDED_TERMS_PARAM,
          true);
        boolean enable_position_increments = jsonQuery.optBoolean(ENABLE_POSITION_INCREMENTS_PARAM,
          true);
        ...
        // boolean auto_generate_phrase_queries =
        // jsonQuery.optBoolean(AUTO_GENERATE_PHRASE_QUERIES_PARAM, false);
        ...
        for (String field : fields) {
          QueryParser qparser = new QueryParser(Version.LUCENE_43, field, _qparser.getAnalyzer());
