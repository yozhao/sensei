...
public class NullFilterConstructor extends FilterConstructor {
    ...
    return new Filter() {
      ...
      public DocIdSet getDocIdSet(AtomicReaderContext context, Bits acceptDocs) throws IOException {
        ...
        throw new UnsupportedOperationException(
            "The null filter is supported only for the bobo facetHandlers that use FacetDataCache");
      }
    };
  }
  ...
  public final static class MultiValueFacetDocIdSetIterator extends FacetDocIdSetIterator {
    ...
    final public int nextDoc() throws IOException {
      return (_doc = (_doc < _maxID ? _nestedArray.findValue(_index, (_doc + 1), _maxID, true)
          : NO_MORE_DOCS));
    }
    ...
    final public int advance(int id) throws IOException {
      if (_doc < id) {
        return (_doc = (id <= _maxID ? _nestedArray.findValue(_index, id, _maxID, true)
            : NO_MORE_DOCS));
      }
      return nextDoc();
    }
