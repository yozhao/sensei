...
public class SenseiTermFilter extends Filter {
  ...
  public SenseiTermFilter(String name, String vals[], String[] not, boolean isAnd,
      boolean noAutoOptimize) {
    _name = name;
  ...
  private static DocIdSet buildLuceneDefaultDocIdSet(final BoboSegmentReader reader,
    ...
    if (vals != null && vals.length > 0) positiveSet = buildDefaultDocIdSets(reader, name, vals,
      isAnd);

    if (nots != null && nots.length > 0) negativeSet = buildDefaultDocIdSets(reader, name, nots,
      false);
    ...
    if (positiveSet != null) {
      if (negativeSet == null) {
        ...
        DocIdSet[] sets = new DocIdSet[] { positiveSet,
            new NotDocIdSet(negativeSet, reader.maxDoc()) };
    ...
    if (context.reader() instanceof BoboSegmentReader) {
      ...
      if (facetHandler != null) {
        ...
        if (_noAutoOptimize && obj != null && obj instanceof FacetDataCache) {
          ...
          return buildLuceneDefaultDocIdSet(boboReader, _name, validVals.toArray(new String[0]),
            _not, _isAnd);
        }
        // we get to optimize using facets
        BrowseSelection sel = new BrowseSelection(_name);
        sel.setValues(_vals);
        if (_not != null) sel.setNotValues(_not);
        if (_isAnd) sel.setSelectionOperation(ValueOperation.ValueOperationAnd);
        else sel.setSelectionOperation(ValueOperation.ValueOperationOr);
