...
public class BooleanFilterConstructor extends FilterConstructor {
  ...
  protected Filter doConstructFilter(Object param) throws Exception {
    ...
    if (obj != null) {
      if (obj instanceof JSONArray) {
        for (int i = 0; i < ((JSONArray) obj).length(); ++i) {
          andFilters.add(new NotFilter(FilterConstructor.constructFilter(
            ((JSONArray) obj).getJSONObject(i), _qparser)));
        }
      } else if (obj instanceof JSONObject) {
        andFilters
            .add(new NotFilter(FilterConstructor.constructFilter((JSONObject) obj, _qparser)));
      }
    }
    ...
    if (array != null) {
      ...
      if (orFilters.size() > 0) andFilters.add(new OrFilter(orFilters));
    }

    if (andFilters.size() > 0) return new AndFilter(andFilters);
    else return null;
  }
}
