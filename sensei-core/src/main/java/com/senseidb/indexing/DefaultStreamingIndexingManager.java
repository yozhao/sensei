...
public class DefaultStreamingIndexingManager implements SenseiIndexingManager<JSONObject> {
  ...
  static {
    try {
      MetricName providerBatchSizeMetricName = new MetricName(MetricsConstants.Domain, "meter",
          "provider-batch-size", "indexing-manager");
      ProviderBatchSizeMeter = Metrics.newMeter(providerBatchSizeMetricName, "provide-batch-size",
        TimeUnit.SECONDS);

      MetricName updateBatchSizeMetricName = new MetricName(MetricsConstants.Domain, "meter",
          "update-batch-size", "indexing-manager");
      UpdateBatchSizeMeter = Metrics.newMeter(updateBatchSizeMetricName, "update-batch-size",
        TimeUnit.SECONDS);

      MetricName eventMeterMetricName = new MetricName(MetricsConstants.Domain, "meter",
          "indexing-events", "indexing-manager");
      EventMeter = Metrics.newMeter(eventMeterMetricName, "indexing-events", TimeUnit.SECONDS);

      MetricName indexingLatencyMetricName = new MetricName(MetricsConstants.Domain, "timer",
          "indexing-latency", "indexing-manager");
      IndexingLatencyTimer = Metrics.newTimer(indexingLatencyMetricName, TimeUnit.MILLISECONDS,
        TimeUnit.SECONDS);
    } catch (Exception e) {
  ...
  private Map<Integer, Zoie<BoboSegmentReader, JSONObject>> _zoieSystemMap;
  ...
  public DefaultStreamingIndexingManager(SenseiSchema schema, Configuration senseiConfig,
      SenseiPluginRegistry pluginRegistry, SenseiGateway<?> gateway,
      ShardingStrategy shardingStrategy, PluggableSearchEngineManager pluggableSearchEngineManager) {
    _dataProvider = null;
    ...
    if (_gateway != null) {
      try {
        dataProvider = _gateway.buildDataProvider(_senseiSchema, _oldestSinceKey, pluginRegistry,
          _shardingStrategy, _zoieSystemMap.keySet());
      ...
      try {
        StandardMBean dataProviderMbean = new StandardMBean(new DataProviderAdmin(dataProvider),
            DataProviderAdminMBean.class);
  ...
  public void syncWithVersion(long timeToWait, String version) throws ZoieException {
    ...
    while (itr.hasNext()) {
      ...
      Zoie<BoboSegmentReader, JSONObject> dataConsumer = _zoieSystemMap.get(part_num);
  ...
  private class DataDispatcher implements DataConsumer<JSONObject> {
    ...
    private JSONObject rewriteData(JSONObject event, int partNum) {
      ...
      if (SenseiSchema.EVENT_TYPE_UPDATE.equalsIgnoreCase(type)) {
        ...
        try {
          ...
          for (ZoieMultiReader<BoboSegmentReader> reader : readers) {
    ...
    @Override
    public void consume(Collection<proj.zoie.api.DataConsumer.DataEvent<JSONObject>> data)
      ...
      try {
        for (DataEvent<JSONObject> dataEvt : data) {
          ...
          if (pluggableSearchEngineManager != null
              && pluggableSearchEngineManager.acceptEventsForAllPartitions()) {
          ...
          if (partDataSet != null) {
            ...
            if (rewrited != null) {
              if (rewrited != obj) dataEvt = new DataEvent<JSONObject>(rewrited,
                  dataEvt.getVersion(), dataEvt.isDelete());
        ...
        while (it.hasNext()) {
          ...
          Zoie<BoboSegmentReader, JSONObject> dataConsumer = _zoieSystemMap.get(part_num);
          if (dataConsumer != null) {
            LinkedList<DataEvent<JSONObject>> partDataSet = (LinkedList<DataEvent<JSONObject>>) _dataCollectorMap
                .get(part_num);
            if (partDataSet != null) {
              if (partDataSet.size() == 0) {
                ...
                partDataSet.add(new DataEvent<JSONObject>(markerObj, _currentVersion));
                  ...
                  && !_currentVersion.equals(partDataSet.getLast().getVersion())) {
                DataEvent<JSONObject> last = partDataSet.pollLast();
                partDataSet.add(new DataEvent<JSONObject>(last.getData(), _currentVersion, last
                    .isDelete()));
              }
