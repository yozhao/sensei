...
public class SpringSenseiPluginFactory implements SenseiPluginFactory<List<?>> {
  ...
  @Override
  public synchronized List<?> getBean(Map<String, String> initProperties, String fullPrefix,
      SenseiPluginRegistry pluginRegistry) {
    if (context == null) {
      if (!initProperties.containsKey(SPRING_FILENAME)) {
        throw new IllegalArgumentException("The configuration doesn't contain the property - "
            + SPRING_FILENAME);
      }
      ...
      if (localName.contains("/") || localName.contains("\\")) {
        ...
        File directory = ((FileConfiguration) pluginRegistry.getConfiguration()).getFile()
            .getParentFile();
