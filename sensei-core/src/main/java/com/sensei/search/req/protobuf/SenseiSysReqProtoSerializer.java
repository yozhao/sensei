...
public class SenseiSysReqProtoSerializer {
  ...
  public byte[] responseToBytes(SenseiSystemInfo response) {
    return SenseiSysResultBPO.SysResult.newBuilder().setVal(serialize(response)).build()
        .toByteArray();
  }
  ...
  public SenseiRequest requestFromBytes(byte[] request) {
    try {
      return (SenseiRequest) ProtoConvertUtil.serializeIn(SenseiRequestBPO.Request.newBuilder()
  ...
  public SenseiSystemInfo responseFromBytes(byte[] result) {
    try {
      return (SenseiSystemInfo) ProtoConvertUtil.serializeIn(SenseiSysResultBPO.SysResult
          .newBuilder().mergeFrom(result).build().getVal());
    } catch (TextFormat.ParseException e) {
      throw new RuntimeException(e);
    } catch (InvalidProtocolBufferException e) {
      throw new RuntimeException(e);
