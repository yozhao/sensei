...
import com.senseidb.indexing.hadoop.map.SenseiHBaseMapper;
...
public class HBaseMapReduceJob extends Configured {

  private static final NumberFormat NUMBER_FORMAT = NumberFormat.getInstance();
  private static final Logger logger = Logger.getLogger(HBaseMapReduceJob.class);
  ...
  public JobConf createJob(Class MRClass) throws IOException, URISyntaxException {
    ...
    String tableName = conf.get(SenseiJobConfig.HBASE_TABLE_NAME, null);
    logger.info("tableName: " + tableName);
    String columns = conf.get(SenseiJobConfig.HBASE_COLUMNS, null);
    ...
    if (fs.exists(new Path(conf.get(MRConfig.TEMP_DIR)))) fs.delete(
      new Path(conf.get(MRConfig.TEMP_DIR)), true);
    ...
    if (schemaFile == null) throw new IOException("no schema file is found");
    else {
    ...
    if (jobConf.getJobName().length() < 1) jobConf.setJobName(MRClass.getName() + "_"
    ...
    TableMapReduceUtil.initTableMapJob(tableName, columns, SenseiHBaseMapper.class, Shard.class,
      IntermediateForm.class, jobConf);
  ...
  private static Shard[] createShards(String indexPath, int numShards,
    ...
    if (fs.exists(path)) {
      ...
      for (int i = 0; i < shards.length; i++) {
        shards[i] = new Shard(versionNumber, parent + indexSubDirPrefix + NUMBER_FORMAT.format(i),
            generation);
      }
      return shards;
    }
