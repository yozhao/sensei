...
public class SenseiHBaseMapper extends MapReduceBase implements TableMap<Shard, IntermediateForm> {

  private final static Logger logger = Logger.getLogger(SenseiHBaseMapper.class);
  ...
  public void configure(JobConf job) {
    ...
    _shardingStategy = (ShardingStrategy) ReflectionUtils.newInstance(job.getClass(
      SenseiJobConfig.DISTRIBUTION_POLICY, DummyShardingStrategy.class, ShardingStrategy.class),
      job);

    _converter = (MapInputConverter) ReflectionUtils.newInstance(job.getClass(
